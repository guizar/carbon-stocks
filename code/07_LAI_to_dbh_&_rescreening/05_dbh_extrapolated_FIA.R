...
## transform to spdf
FIA_extr = FIA_extr[,c("LON","LAT","PFT6","PFT8","PFT9","PFT10","PFT11")]

colnames(FIA_extr)[2] = "y"
colnames(FIA_extr)[1] = "x"

coordinates(FIA_extr) <- ~x+y

## see what's happening
plot(roiPoly)
title("BBOX & extrapolated FIA")
plot(FIA_extr,add=T,pch=18, cex=0.2)


### get closest match
library(FNN)
closestExtrap = get.knnx(data=FIA_extr@coords, query=LAI_3m_df[1:2],k = 1)
